...
* BY:		Eric Hollas																	*
*																						*
* FILE:		PaddleMoveNegativeState.h													*
...
#ifndef STATE_PADDLEMOVENEGATIVE
#define STATE_PADDLEMOVENEGATIVE
...
class State_PaddleMoveNegative : public state<Character_Paddle> {
public:
	/*
	* Function: execute
	*
	...
	*				paddle moves in the negative direction, only moves once
	...
	void onMessage(Character_Paddle * p_character, message &msg) override {
		...
		case collision_check:
			...
			if (isCollision) {
				...
				DISPATCH->collect(collision_happened, msg.sender_id, p_character->getID(), msg.post_script.position, p_character->getRadius(), dist);
			}
			break;
		case move_up:
			//changes state
	...
	*
	*/
	static State_PaddleMoveNegative* instance();
};

#endif /*STATE_PADDLEMOVENEGATIVE*/

State_PaddleMoveNegative* State_PaddleMoveNegative::instance() {
	static State_PaddleMoveNegative inst;
	return &inst;
}